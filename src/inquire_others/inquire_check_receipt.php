...
function gotoUrl(target){
    ...
    var url = "../receipt_check/receipt_show_check.php?id="+id;
    //location.href = url;
...
<input name="search" type="button" value="查找" onclick="url='../receipt_check/receipt_show_check.php?id=' + document.getElementById('receipt_id').value;window.open(url,'_blank')"/>
</div>
<!--精确查询框架-->
<!--查询框架-->
<div style="margin-bottom:8px; height:20px">
...
<table id="MyTable" width="100%" border="1" cellspacing="0" cellpadding="5" bordercolor="#9999FF">
  ...
  <?php
    while($RS = mysql_fetch_array($result_receipt))
    {
        ...
        echo "<td customvalue='$RS[id]'><a href='../receipt_check/receipt_show_check.php?id=$RS[id]'>$RS[id]</a></td>\n";
        echo "<td>$RS[date]</td>\n";
