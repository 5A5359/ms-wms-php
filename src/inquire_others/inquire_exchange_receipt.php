...
function gotoUrl(target){
    ...
    var url = "../receipt_exchange/receipt_show_exchange.php?id="+id;
    //location.href = url;
...
function optionChange(object){
    ...
    switch(object.value){
        ...
        case 'inquiry_warehouse2':
            document.getElementById('inquiry_warehouse2').style.display = "inline";
            break;
        default:break;
    }
}
function hideAll(){
    ...
    document.getElementById('inquiry_warehouse2').style.display = "none";
}
</script>
...
<div style="margin-bottom:8px; height:20px"> 精确查询：
  ...
  <input name="search" type="button" value="查找" onclick="url='../receipt_exchange/receipt_show_exchange.php?id=' + document.getElementById('receipt_id').value;window.open(url,'_blank')"/>
</div>
<!--精确查询框架-->
<!--查询框架-->
...
<table id="MyTable" width="100%" border="1" cellspacing="0" cellpadding="5" bordercolor="#9999FF">
  ...
  <tbody>
    ...
    while($RS = mysql_fetch_array($result_receipt))
    {
        ...
        echo "<td customvalue='$RS[id]'><a href='../receipt_exchange/receipt_show_exchange.php?id=$RS[id]'>$RS[id]</a></td>\n";
        echo "<td>$RS[date]</td>\n";
